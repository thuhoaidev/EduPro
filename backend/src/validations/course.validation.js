...
const createCourseSchema = Joi.object({
  ...
  level: Joi.string().required().valid('beginner', 'intermediate', 'advanced').messages({
  ...
  discount_amount: Joi.number().min(0).default(0).messages({
    'number.base': 'Giảm giá theo số tiền phải là số',
    'number.min': 'Giảm giá theo số tiền không được âm',
  }),
  discount_percentage: Joi.number().min(0).max(100).default(0).messages({
...
const updateCourseStatusSchema = Joi.object({
  status: Joi.string().valid('draft', 'pending', 'approved', 'rejected').messages({
    'any.only': 'Trạng thái phải là draft, pending, approved hoặc rejected',
  }),
  displayStatus: Joi.string().valid('hidden', 'published').messages({
