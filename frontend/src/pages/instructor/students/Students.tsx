...
// 1. Định nghĩa interface cho Student
interface Student {
  id: number;
  name: string;
  email: string;
...
const StudentListPage: React.FC = () => {
  // 2. Khai báo rõ ràng useState có kiểu là Student[]
  const [students, setStudents] = useState<Student[]>([]);
  const navigate = useNavigate();

  useEffect(() => {
    // 3. Mock data thuộc kiểu Student[]
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: Student) => (
        <Button
          type="link"
          onClick={() => navigate(`/instructor/students/${record.id}`)}
        >
          Xem chi tiết
        </Button>
      ),
