import React from 'react';
import { Layout, Input, Space, Button, Avatar, Badge } from 'antd';
import { SearchOutlined, BellOutlined, UserOutlined, ShoppingCartOutlined } from '@ant-design/icons';

// Assuming you have a Udemy logo file, replace with the correct path
// import udemyLogo from '../../assets/images/udemy-logo.png'; // Placeholder

const { Header } = Layout;

// Removed unused menu definition

const AppHeader = () => {
  return (
    // Removed the top bar div
    <Header style={{
      background: '#fff',
      padding: '0 24px',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
      borderBottom: '1px solid #f0f0f0',
      boxShadow: '0 2px 8px rgba(0, 0, 0, 0.05)',
      height: 70, // Adjust height to match image
    }} className="sticky top-0 z-50 w-full">
      <div className="flex items-center h-full">
        {/* Logo */}
        {/* Replace with actual Udemy logo */}
        <img src="https://www.udemy.com/staticx/udemy/images/v7/logo-udemy.svg" alt="Udemy Logo" style={{
          height: 34, // Adjust size to match image
          marginRight: 8,
        }} />
        {/* Explore Text (not a dropdown in the image) */}
        <span className="font-semibold text-gray-700 cursor-pointer hidden md:block">Explore</span> {/* Hidden on small screens */}
      </div>

      {/* Search Input */}
      <div className="flex-1 mx-4 max-w-md">
        <Input
          placeholder="Find your next course by skill, topic, or instructor"
          prefix={<SearchOutlined style={{ color: 'rgba(0,0,0,.45)' }} />}
          style={{
            width: '100%',
            borderRadius: 9999, // Fully rounded
            padding: '10px 20px',
            border: '1px solid #d1d7dc', // Lighter border color
            backgroundColor: '#f7f9fa', // Light background
          }}
          className="hover:border-[#a435f0] focus:border-[#a435f0]"
          onFocus={(e) => {
            e.target.style.borderColor = '#a435f0'; // Udemy purple on focus
            e.target.style.boxShadow = '0 0 0 1px rgba(164, 53, 240, 0.2)';
            e.target.style.backgroundColor = '#fff';
          }}
          onBlur={(e) => {
            e.target.style.borderColor = '#d1d7dc';
            e.target.style.boxShadow = 'none';
             e.target.style.backgroundColor = '#f7f9fa';
          }}
        />
      </div>

      {/* Right Section: Links and Icons */}
      <Space size="middle" className="h-full flex items-center">
        {/* Links */}
        <Button type="link" className="text-gray-700 font-semibold hidden lg:block">Udemy Business</Button>
        <Button type="link" className="text-gray-700 font-semibold hidden lg:block">Teach on Udemy</Button>
        <Button type="link" className="text-gray-700 font-semibold hidden md:block">My learning</Button> {/* Show on md and up */}

        {/* Cart Icon */}
        {/* Add Badge for notification count if needed */}
        <Button type="text" icon={<ShoppingCartOutlined style={{ fontSize: '1.4em', color: '#5624d0' }} />} className="text-gray-700" />

        {/* Bell Icon */}
         <Button type="text" icon={<BellOutlined style={{ fontSize: '1.4em', color: '#5624d0' }} />} className="text-gray-700" />

        {/* User Avatar with Notification Dot */}
        <Badge dot offset={[-2, 2]} size="small">
           <Avatar icon={<UserOutlined />} style={{
             backgroundColor: '#000', // Black background for avatar
             color: '#fff',
             cursor: 'pointer',
             height: 40, width: 40,
             display: 'flex', alignItems: 'center', justifyContent: 'center',
           }} />
        </Badge>
      </Space>
    </Header>
  );
};

export default AppHeader;